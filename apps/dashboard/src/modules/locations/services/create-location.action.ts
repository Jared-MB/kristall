...
export async function createLocation(
	...
	const parsedData =
		LOCATIONS_ROUTES["create-location"].clientInput.safeParse(values);
	...
	const [error, response] = await locationsClient.POST("create-location", {
	...
	// Only on the success path: expiring the tag ships a re-render of the route
	// in this action's response, so the caller gets the new list and the form
